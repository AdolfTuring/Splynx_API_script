...
//file to array
$csv = array_map('str_getcsv', file('Aspen.csv')); //csv-file to array
//print_r($csv);
...
foreach ($csv as $file) {
        ...
        if ($result) {
            file_put_contents($log_file, "Customer's {$file[1]} service ID:{$file[0]} updated, IP {$file[2]}\n", FILE_APPEND);
      
        } else {
            file_put_contents($log_file, "ERROR! Customer's {$file[1]} service ID:{$file[0]} NOT updated, IP {$file[2]}, {$api->response_code}\n", FILE_APPEND);
